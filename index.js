...
async function CurrencyConversions() {
    ...
    console.log(chalk.bold(chalk.greenBright(`
      ...
      `)));
    const CurrencyExchanger = await inquirer.prompt([
        {
    ...
    try {
        ...
        if (CurrencyAmount.result === "success" && ConvertedRates !== undefined) {
            console.log(chalk.bold(chalk.blueBright(`${ConvertionAmount} ${ConvertCurrency} To ${ConvertionCurrency} = (${convertedAmount})`)));
        }
        else {
